...
export default function Menu({ devices }: MenuProps) {
  ...
  {
    hasAlerts && isExplore && (
      <span className="pointer-events-none absolute top-1 right-1 h-2.5 w-2.5 rounded-full bg-red-500 ring-2 ring-white" />
    );
  }

  useEffect(() => {
    if (!user) return;
    fetch("/api/sensor-alerts/unseen-status")
      .then((res) => res.json())
      .then((data) => setHasAlerts(data.hasUnseen))
      .catch(() => {});
  }, [user]);
